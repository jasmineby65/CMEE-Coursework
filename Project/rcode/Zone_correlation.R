...
for(i in 1:length(unique(data$Variable))){
  ...
  model <- lmer(data = subset, Kendall ~ Zone + (1|Model))
...
# 
# write_csv(res.kruskal_correlation, "../csv/Zonal_correlation_export.csv")
...
Zone <- ggplot(data=data, aes(x = Zone, y=Kendall)) +
  geom_boxplot(lwd=0.5, fill = "#FFCC33") +
  theme_classic() +
  ggtitle("(A) Correlation of percentage change with export in increasing and decreasing export zones") +
...
# Not box #
exp <- ggplot(data=data, aes(x = Zone, y=Kendall, col = Model)) +
  geom_point(size = 10)+
  theme_classic() +
  ggtitle("(A) Correlation of percentage change with export in increasing and decreasing export zones") +
  labs(y = "Kendall rank correlation coefficient")  +
...
Zone2 <- ggplot(data=data, aes(x = Zone, y=Kendall)) +
  geom_boxplot(lwd=0.5, fill = "darkslategray3") +
  theme_classic() +
  ggtitle("(C) Correlation of percentage change with sea ice fraction in increasing and decreasing export zones") +
  labs(y = "Kendall rank correlation coefficient")  +
  theme(plot.title = element_text(size = 18, face = "bold", margin=margin(0,0,15,0)),
        plot.title.position = "plot",
        strip.text.x = element_text(size = 13, hjust = 0.5, face = "bold", color = "#333333"), 
...
# Not box #
ice <- ggplot(data=data, aes(x = Zone, y=Kendall, col = Model)) +
  geom_point(size = 10)+
  theme_classic() +
  ggtitle("(C) Correlation of percentage change with sea ice fraction in increasing and decreasing export zones") +
  labs(y = "Kendall rank correlation coefficient")  +
...
# res.kruskal_correlation
# 
# write_csv(res.kruskal_correlation, "../csv/Zonal_correlation_npp.csv")
...
Zone3 <- ggplot(data=data, aes(x = Zone, y=Kendall)) +
  geom_boxplot(lwd=0.5, fill = "olivedrab3") +
  theme_classic() +
  ggtitle("(B) Correlation of percentage change with NPP in increasing and decreasing export zones") +
  labs(y = "Kendall rank correlation coefficient")  +
  theme(plot.title = element_text(size = 18, face = "bold", margin=margin(0,0,15,0)),
        plot.title.position = "plot",
        strip.text.x = element_text(size = 13, hjust = 0.5, face = "bold", color = "#333333"), 
...
# Not box #
npp <- ggplot(data=data, aes(x = Zone, y=Kendall, col = Model)) +
  geom_point(size = 10)+
  theme_classic() +
  ggtitle("(B) Correlation of percentage change with NPP in increasing and decreasing export zones") +
  labs(y = "Kendall rank correlation coefficient")  +
...
###### Final plot #####
png(filename="../output/Multimodel/Final_zone_correlatiom.png", res=600, width=10000, height=12000)
grid.arrange(Zone, Zone3, Zone2, ncol = 1)
dev.off()

png(filename="../output/Multimodel/Fial_zone_correlatiom_point.png", res=600, width=10000, height=12000)
